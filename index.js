import{a as i,i as l,R as $}from"./assets/vendor-CaGxMn-M.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function n(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(s){if(s.ep)return;s.ep=!0;const a=n(s);fetch(s.href,a)}})();const N=document.querySelector(".js-open-menu"),R=document.querySelector(".js-close-menu"),y=document.querySelector(".js-menu"),k=document.body,j=document.querySelectorAll(".mobile-menu__link");N.addEventListener("click",()=>{y.classList.add("is-open"),k.classList.add("no-scroll")});R.addEventListener("click",()=>{y.classList.add("closing"),k.classList.remove("no-scroll"),setTimeout(()=>{y.classList.remove("is-open","closing")},200)});j.forEach(e=>{e.addEventListener("click",()=>{y.classList.remove("is-open"),k.classList.remove("no-scroll")})});const q=document.querySelector(".loader");function O(){q.classList.remove("is-hidden")}function G(){q.classList.add("is-hidden")}const b="/artists_hub_project/assets/icon-sprite-CX7i4XyO.svg";i.defaults.baseURL="https://sound-wave.b.goit.study/api";document.querySelector(".loader");const x=e=>{const t=Math.floor(e/6e4),n=Math.floor(e%6e4/1e3);return`${t}:${n<10?"0":""}${n}`};async function D(e){if(!e){l.error({title:"Error",message:"Artist ID is missing."});return}H();try{const t=await i.get(`/artists/${e}`),o=(await i.get(`/artists/${e}/albums`)).data.albumsList||[],s=t.data,a=M("artists");document.body.appendChild(a),w(a),V(a,s,o)}catch(t){console.error("Error fetching artist details:",t),l.error({title:"Error",message:"Failed to load artist details."})}finally{P()}}function M(e){const t=document.createElement("div");t.className=`${e}-modal`;const n=window.scrollY;return document.body.style.position="fixed",document.body.style.top=`-${n}px`,t.innerHTML=`
  ...
  `,t}function H(){let e=document.querySelector(".global-loader");e||(e=document.createElement("div"),e.className="global-loader",e.innerHTML='<div class="loader"></div>',document.body.appendChild(e)),e.classList.remove("hidden")}function P(){const e=document.querySelector(".global-loader");e&&e.classList.add("hidden")}function w(e,t){const n=e.querySelector(".modal-close-btn"),o=e.querySelector(".fb-modal-close-btn");function s(){e.remove();const c=document.body.style.top;document.body.style.position="",document.body.style.top="",window.scrollTo(0,parseInt(c||"0")*-1),typeof t=="function"&&t(),document.removeEventListener("keydown",a),e.removeEventListener("click",r)}n&&n.addEventListener("click",s),o&&o.addEventListener("click",s);const a=c=>{c.key==="Escape"&&s()};document.addEventListener("keydown",a);const r=c=>{c.target===e&&s()};e.addEventListener("click",r)}function V(e,t,n){const o=e.querySelector(".artists-modal-content");o.innerHTML=`
            <span class="modal-close-btn-wraper">
            <button type="button" class="modal-close-btn" aria-label="Close"> <svg class="SVG-icon"
              width="24" height="16">
              <use href="${b}#icon-close"></use>
    ...
    `,w(e)}i.defaults.baseURL="https://sound-wave.b.goit.study/api";const v=document.querySelector(".artists-list"),g=document.querySelector(".load-more-btn");let u=1;const I=8,d={genre:null,sortName:null,name:null},Y=document.querySelector(".reset-btn"),L=document.querySelectorAll(".dropdown"),B=document.querySelector(".genre-list"),U=document.querySelector(".sort-list"),E=document.querySelector(".search-input"),_=document.querySelector(".search-btn"),z=document.querySelector(".main-dropdown-btn"),T=document.querySelector(".filters-container");async function X(){const{data:e}=await i.get("/genres");return e}async function K(){const t=(await X()).map(n=>`<li data-value="${n.genre}">${n.genre}</li>`).join("");B.insertAdjacentHTML("beforeend",t)}K();z.addEventListener("click",e=>{T.classList.toggle("is-open")});L.forEach(e=>{e.querySelector(".dropdown-btn").addEventListener("click",()=>{L.forEach(n=>{n!==e&&n.classList.remove("open")}),e.classList.toggle("open")})});B.addEventListener("click",e=>{d.genre=e.target.dataset.value||null,u=1,S(),p()});U.addEventListener("click",e=>{const t=e.target.dataset.value;d.sortName=t,u=1,S(),p()});function S(){L.forEach(e=>e.classList.remove("open"))}function A(){d.name=E.value.trim()||null,u=1,p()}E.addEventListener("keydown",e=>{e.key==="Enter"&&A()});_.addEventListener("click",A);function C(){d.genre=null,d.sortName=null,d.name=null,E.value="",u=1,S(),p()}Y.addEventListener("click",C);p();g.addEventListener("click",()=>{u+=1,p()});v.addEventListener("click",function(e){const t=e.target.closest(".learn-more-btn");if(!t)return;const n=t.dataset.id;D(n)});async function J(e){return(await i.get("/artists",{params:{page:e,limit:I,genre:d.genre||null,sortName:d.sortName||null,name:d.name||null}})).data}function Q(e,t=20){if(!e)return"";const n=e.indexOf("."),o=e.split(" ");if(n!==-1){const s=e.substring(0,n+1);if(s.split(" ").length<=t)return s}return o.length>t?o.slice(0,t).join(" ")+"...":e}function W(e,t=!1){const n=e.map(({_id:o,strArtist:s,strArtistThumb:a,genres:r,strBiographyEN:c})=>`
    ...
    <use href="${b}#icon-icon-lr-more"></use>
  </svg></button>
            </div>
        </li>
    `).join("");t?v.insertAdjacentHTML("beforeend",n):v.innerHTML=n}async function p(){try{O(),g.classList.add("is-hidden");const e=await J(u);if(!e.artists||e.artists.length===0){T.classList.toggle("is-open"),v.innerHTML=` 
      <li class="no-results">
      <svg class="SVG-icon" width="40" height="40">
                    <use href="${b}#error-icon"></use>
                  </svg>
      ...
      </li> `,document.querySelector(".err-reset-btn").addEventListener("click",C);return}W(e.artists,u>1),e.page>=e.totalPages?(g.classList.add("is-hidden"),u>1&&l.info({position:"topRight",message:"Ouch! That is all, folks! No more artists to show."})):g.classList.remove("is-hidden")}catch{l.error({position:"topRight",message:"Welcome to Ukraine! Blackout again? Or the server is resting..."})}finally{G()}}i.defaults.baseURL="https://sound-wave.b.goit.study/api";let h;async function Z(){try{return(await i.get("/feedbacks",{params:{page:1,limit:10}})).data.data||[]}catch(e){return console.error("Error fetching feedbacks:",e),l.error({title:"Error",message:"Failed to load feedbacks."}),[]}}function ee(e){const t=document.querySelector(".swiper-wrapper");t.innerHTML="",e.forEach(n=>{const o=document.createElement("div");o.className="swiper-slide",o.innerHTML=`
            <div class="feedback-card">
            <div class="rating" data-rating="${n.rating||0}"></div>
                <p class="feedback-message">"${n.descr}"</p>
                <p class="feedback-author">${n.name}</p>
            </div>
        `,t.appendChild(o)}),te()}function te(){document.querySelectorAll(".rating").forEach(t=>{const n=parseFloat(t.dataset.rating)||0;new $(t,{readOnly:!0,score:n,number:5,path:"raty-images/",starType:"img"}).init()})}function se(){h&&h.destroy(),h=new Swiper(".mySwiper",{pagination:{el:".swiper-pagination",dynamicBullets:!0},slidesPerView:1,spaceBetween:20,navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"}})}async function ne(){const e=await Z();ee(e),se()}document.addEventListener("DOMContentLoaded",ne);const f=document.querySelector(".leave-feedback-btn");i.defaults.baseURL="https://sound-wave.b.goit.study/api";async function ae(){f.disabled=!0;const e=M("feedback");document.body.appendChild(e);try{oe(e),w(e,()=>{f.disabled=!1})}catch(t){console.error("Error opening feedback modal:",t),l.error({title:"Error",message:"Failed to open feedback form."}),f.disabled=!1}}function oe(e){const t=e.querySelector(".feedback-modal-content");t.innerHTML=`
