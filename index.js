import{a as i,i as c,R as $}from"./assets/vendor-CaGxMn-M.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function n(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(s){if(s.ep)return;s.ep=!0;const a=n(s);fetch(s.href,a)}})();const F=document.querySelector(".js-open-menu"),N=document.querySelector(".js-close-menu"),g=document.querySelector(".js-menu"),k=document.body,R=document.querySelectorAll(".mobile-menu__link");F.addEventListener("click",()=>{g.classList.add("is-open"),k.classList.add("no-scroll")});N.addEventListener("click",()=>{g.classList.add("closing"),k.classList.remove("no-scroll"),setTimeout(()=>{g.classList.remove("is-open","closing")},200)});R.forEach(e=>{e.addEventListener("click",()=>{g.classList.remove("is-open"),k.classList.remove("no-scroll")})});const q=document.querySelector(".loader");function j(){q.classList.remove("is-hidden")}function C(){q.classList.add("is-hidden")}const p="/artists_hub_project/assets/icon-sprite-pKjToWOX.svg";i.defaults.baseURL="https://sound-wave.b.goit.study/api";document.querySelector(".loader");const O=e=>{const t=Math.floor(e/6e4),n=Math.floor(e%6e4/1e3);return`${t}:${n<10?"0":""}${n}`};async function G(e){if(!e){c.error({title:"Error",message:"Artist ID is missing."});return}x();try{const t=await i.get(`/artists/${e}`),r=(await i.get(`/artists/${e}/albums`)).data.albumsList||[],s=t.data,a=M("artists");document.body.appendChild(a),w(a),H(a,s,r)}catch(t){console.error("Error fetching artist details:",t),c.error({title:"Error",message:"Failed to load artist details."})}finally{D()}}function M(e){const t=document.createElement("div");t.className=`${e}-modal`;const n=window.scrollY;return document.body.style.position="fixed",document.body.style.top=`-${n}px`,t.innerHTML=`
  ...
  `,t}function x(){let e=document.querySelector(".global-loader");e||(e=document.createElement("div"),e.className="global-loader",e.innerHTML='<div class="loader"></div>',document.body.appendChild(e)),e.classList.remove("hidden")}function D(){const e=document.querySelector(".global-loader");e&&e.classList.add("hidden")}function w(e,t){const n=e.querySelector(".modal-close-btn");function r(){e.remove();const o=document.body.style.top;document.body.style.position="",document.body.style.top="",window.scrollTo(0,parseInt(o||"0")*-1),typeof t=="function"&&t(),document.removeEventListener("keydown",s),e.removeEventListener("click",a)}n.addEventListener("click",r);const s=o=>{o.key==="Escape"&&r()};document.addEventListener("keydown",s);const a=o=>{o.target===e&&r()};e.addEventListener("click",a)}function H(e,t,n){const r=e.querySelector(".artists-modal-content");r.innerHTML=`
            <span class="modal-close-btn-wraper">
            <button type="button" class="modal-close-btn" aria-label="Close"> <svg class="SVG-icon"
              width="24" height="16">
              <use href="${p}#icon-clouse"></use>
    ...
    `,w(e)}i.defaults.baseURL="https://sound-wave.b.goit.study/api";const h=document.querySelector(".artists-list"),f=document.querySelector(".load-more-btn");let d=1;const P=8,l={genre:null,sortName:null,name:null},V=document.querySelector(".reset-btn"),L=document.querySelectorAll(".dropdown"),B=document.querySelector(".genre-list"),I=document.querySelector(".sort-list"),E=document.querySelector(".search-input"),Y=document.querySelector(".search-btn"),U=document.querySelector(".main-dropdown-btn"),_=document.querySelector(".filters-container");async function z(){const{data:e}=await i.get("/genres");return e}async function K(){const t=(await z()).map(n=>`<li data-value="${n.genre}">${n.genre}</li>`).join("");B.insertAdjacentHTML("beforeend",t)}K();U.addEventListener("click",e=>{_.classList.toggle("is-open")});L.forEach(e=>{e.querySelector(".dropdown-btn").addEventListener("click",()=>{L.forEach(n=>{n!==e&&n.classList.remove("open")}),e.classList.toggle("open")})});B.addEventListener("click",e=>{l.genre=e.target.dataset.value||null,d=1,S(),m()});I.addEventListener("click",e=>{const t=e.target.dataset.value;l.sortName=t,d=1,S(),m()});function S(){L.forEach(e=>e.classList.remove("open"))}function T(){l.name=E.value.trim()||null,d=1,m()}E.addEventListener("keydown",e=>{e.key==="Enter"&&T()});Y.addEventListener("click",T);function A(){l.genre=null,l.sortName=null,l.name=null,E.value="",d=1,S(),m()}V.addEventListener("click",A);m();f.addEventListener("click",()=>{d+=1,m()});h.addEventListener("click",function(e){const t=e.target.closest(".learn-more-btn");if(!t)return;const n=t.dataset.id;G(n)});async function W(e){return(await i.get("/artists",{params:{page:e,limit:P,genre:l.genre||null,sortName:l.sortName||null,name:l.name||null}})).data}function X(e,t=20){if(!e)return"";const n=e.indexOf("."),r=e.split(" ");if(n!==-1){const s=e.substring(0,n+1);if(s.split(" ").length<=t)return s}return r.length>t?r.slice(0,t).join(" ")+"...":e}function J(e,t=!1){const n=e.map(({_id:r,strArtist:s,strArtistThumb:a,genres:o,strBiographyEN:v})=>`
    ...
    `).join("");t?h.insertAdjacentHTML("beforeend",n):h.innerHTML=n}async function m(){try{j(),f.classList.add("is-hidden");const e=await W(d);if(!e.artists||e.artists.length===0){h.innerHTML=` 
      <li class="no-results">
      <svg class="SVG-icon" width="24" height="16">
                    <use href="${p}#icon-warning"></use>
                  </svg>
